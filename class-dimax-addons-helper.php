...
class Helper {
	...
	public static function share_link( $social, $args = array() ) {
		$url  = '';
		$text = esc_html__( 'Share on', 'dimax' ) . ' ' . ucfirst( $social );
		$icon = $social;
		...
		switch ( $social ) {
			...
			case 'googleplus':
				...
				$text = esc_html__( 'Share on Google+', 'dimax' );
				$icon = 'google';
				break;
			...
			case 'pocket':
				...
				$text = esc_html__( 'Save On Pocket', 'dimax' );
				break;
			...
			case 'email':
				$url  = 'mailto:?subject=' . get_the_title() . '&body=' . __( 'Check out this site:', 'dimax' ) . ' ' . get_permalink();
				$text = esc_html__( 'Share Via Email', 'dimax' );
				$icon = 'mail';
				break;
		}
	...
	public static function get_countdown_texts() {
		return apply_filters( 'dimax_get_countdown_texts', array(
			'days'    => esc_html__( 'Days', 'dimax' ),
			'hours'   => esc_html__( 'Hours', 'dimax' ),
			'minutes' => esc_html__( 'Minutes', 'dimax' ),
	...
	public static function get_content_limit( $num_words, $more = "&hellip;", $content = '' ) {
		...
		$content = strip_tags(
			strip_shortcodes( $content ), apply_filters(
				'dimax_content_limit_allowed_tags', '<script>,<style>'
			)
		);
		...
		if ( $more ) {
			$output = sprintf(
				...
				$content,
				get_permalink(),
				sprintf( esc_html__( 'Continue reading &quot;%s&quot;', 'dimax' ), the_title_attribute( 'echo=0' ) ),
				esc_html( $more )
			);
		} else {
