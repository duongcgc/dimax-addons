...
class Product_Meta  {
	...
	const TAXONOMY_TYPE   = 'dimax_product_tab_type';
	...
	public function product_data_tab( $tabs ) {
		...
		$tabs['dimax_product_tabs'] = array(
			'label'    => esc_html__( 'Product Tabs', 'dimax' ),
			'target'   => 'dimax-product-tabs',
			'class'    => array( 'dimax-product-tabs', ),
			'priority' => 62,
		);
	...
	public function product_data_panel() {
		...
		global $post;
		?>
		<div id="dimax-product-tabs" class="panel wc-metaboxes-wrapper woocommerce_options_panel">
			<div class="wc-metaboxes">
				...
				<?php
					...
					$tab_title = $tab_disable ? $tab_title . '(' . esc_html__( 'Disabled', 'dimax' ) . ')' : $tab_title;
				...
				<div class="wc-metabox postbox">
					<h3 class="<?php echo esc_attr($tab_class); ?>" style="border-bottom: 1px solid #ccc">
						<div class="handlediv" title="<?php esc_html_e('Click to toggle', 'dimax'); ?>"></div>
					...
					<div class="wc-metabox-content" style="padding: 20px">
						...
						woocommerce_wp_checkbox(
							array(
								...
								'label'       => esc_html__('Tab Disable', 'dimax' ),
								'value'       => wc_bool_to_string( $tab_disable ),
							)
						);?>
						<fieldset class="form-field">
							<label><?php esc_html_e( 'Tab Content', 'dimax' ); ?></label>
							<?php
