jQuery(document).ready(function ($) {
	...
	var $tab_default = $('.wp-list-table .dimax_product_tab_type-default');
	$tab_default.find('.check-column input[type="checkbox"]').remove();

	$( '#the-list' ).find('.dimax_product_tab_type-default').each(function() {
	...
	var $tab_settings = $('#dimax-product-tabs-settings'),
		$tab_type = $tab_settings.find('.dimax-product-tab--input'),
		$tab_product = $tab_settings.find('.dimax-product-tabs--product'),
		$tab_categories = $tab_settings.find('.dimax-product-tabs--categories');
	...
	$( 'table.widefat tbody' ).sortable({
		...
		update: function( event, ui ) {
			...
			$.post(
				ajaxurl,
				{ action: 'dimax_product_tab_ordering', id: postid, previd: prevpostid, nextid: nextpostid },
				function( response ) {
