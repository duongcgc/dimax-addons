jQuery(document).ready(function ($) {
	...
	// Render tabs.
	var $tabs = $( '#dimax-size-guide-tabs' ),
		$tabsNav = $( '.dimax-size-guide-tabs--tabs', $tabs ),
		$addTab = $( '.add-new-tab', $tabsNav ),
		templates = {
			tab: wp.template( 'dimax-size-guide-tab' ),
			panel: wp.template( 'dimax-size-guide-panel' )
		};

	if ( ! _.isUndefined( dimaxSizeGuideTables ) ) {
		for ( var i = 0; i < dimaxSizeGuideTables.tables.length; i++ ) {
			$addTab.before( templates.tab( {
				index: i,
				tab: dimaxSizeGuideTables.tabs[i]
			} ) );

			$tabs.append( templates.panel( {
				index: i,
				name: dimaxSizeGuideTables.names[i],
				description: dimaxSizeGuideTables.descriptions[i],
				table: dimaxSizeGuideTables.tables[i],
				information: dimaxSizeGuideTables.information[i]
			} ) );

			$tabs.find( '.dimax-size-guide-table-editor[data-tab='+ i +']' ).find( '.dimax-size-guide-table' ).dimaxEditTable();

			$addTab.data( 'index', i );

			if ( 0 === i ) {
				$tabsNav.children( ':eq(0)' ).addClass( 'active' );
				$tabs.find( '.dimax-size-guide-table-editor[data-tab='+ i +']' ).addClass( 'active' );
			}
	...
	// Change active tab.
	$tabsNav.on( 'click', '.dimax-size-guide-table-tabs--tab', function() {
		var $el = $( this );
		...
		$tabs.find( '.dimax-size-guide-table-editor[data-tab='+ $el.data( 'tab' ) +']' ).addClass( 'active' ).siblings( '.active' ).removeClass( 'active' );
	} );

	// Edit tab title.
	$tabsNav
		.on( 'click', '.edit-button', function() {
			$( this ).closest( '.dimax-size-guide-table-tabs--tab' ).addClass( 'editting' ).children( 'input' ).removeClass( 'hidden' );
		} )
		.on( 'click', '.confirm-button', function() {
			$( this ).closest( '.dimax-size-guide-table-tabs--tab' ).removeClass( 'editting' ).children( 'input' ).addClass( 'hidden' );
		} )
		.on( 'input', 'input', function() {
			$( this ).siblings( '.dimax-size-guide-table-tabs--tab-text' ).text( this.value );
		} );
	...
	$tabsNav.on( 'click', '.add-new-tab', function() {
		...
		$tabs.find( '.dimax-size-guide-table-editor[data-tab='+ index +']' ).find( '.dimax-size-guide-table' ).dimaxEditTable();
	...
	$tabs.on( 'click', '.delete-table', function( event ) {
		...
		var $deletedPanel = $( this ).closest( '.dimax-size-guide-table-editor' ),
	...
	$tabs.on( 'tab_added tab_removed', function() {
		if ( $tabs.children( '.dimax-size-guide-table-editor' ).length <= 1 ) {
...
(function ($, window, i) {
	...
	$.fn.dimaxEditTable = function (options) {

		// Settings
		var s = $.extend({
				data: [ ['']],
				tableClass: 'dimax-table-edit',
				jsonData: false,
				headerCols: false,
				maxRows: 999,
				first_row: true,
