...
class Frontend {
	...
	public function scripts() {
		wp_enqueue_style( 'dimax-size-guide-content', RAZZI_ADDONS_URL . 'modules/size-guide/assets/css/size-guide.css', array(), '1.0' );
		wp_enqueue_script('dimax-size-guide-content', RAZZI_ADDONS_URL . 'modules/size-guide/assets/js/size-guide-tab.js');
	}
	...
	public function display_size_guide() {
		...
		$guide_settings = get_post_meta( $product->get_id(), 'dimax_size_guide', true );
	...
	public function size_guide_tab( $tabs ) {
		...
		$button_text = $text ? $text : esc_html__('Size Chart' , 'dimax');
	...
	public function size_guide_content() {
		...
		echo '<div class="dimax-size-guide">';

		if ( ! empty( $guide->post_content ) ) {
			echo '<div class="dimax-size-guide--global-content">' . apply_filters( 'the_content', $guide->post_content ) . '</div>';
		}
		...
		if ( 1 < count( $size_guides['tables'] ) ) {
			...
			echo '<div class="dimax-size-guide-tabs">';
			echo '<ul class="dimax-size-guide-tabs__nav">' . implode( '', $tabs ) . '</ul>';
			echo '<div class="dimax-size-guide-tabs__panels">';
		}

		foreach ( $size_guides['tables'] as $index => $table ) {
			echo '<div class="dimax-size-guide-tabs__panel ' . ( $index ? '' : 'active' ) . '" data-panel="' . esc_attr( $index + 1 ) . '">';

			if ( ! empty( $size_guides['names'][ $index ] ) ) {
				echo '<h4 class="dimax-size-guide__name">' . wp_kses_post( $size_guides['names'][ $index ] ) . '</h4>';
			}

			if ( ! empty( $size_guides['descriptions'][ $index ] ) ) {
				echo '<div class="dimax-size-guide__description">' . wp_kses_post( $size_guides['descriptions'][ $index ] ) . '</div>';
			}

			if ( ! empty( $table ) ) {
				$table = json_decode( $table, true );

				echo '<table class="dimax-size-guide__table">';
			...
			if ( ! empty( $size_guides['information'][ $index ] ) ) {
				echo '<div class="dimax-size-guide__info">' . wp_kses_post( $size_guides['information'][ $index ] ) . '</div>';
			}
	...
	public function get_product_size_guide_id( $object = false ) {
		...
		$size_guide = get_post_meta( $_product->get_id(), 'dimax_size_guide', true );
