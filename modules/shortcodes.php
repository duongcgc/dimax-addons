...
class Shortcodes {
	...
	public function __construct() {
		add_shortcode( 'dimax_product_taxonomy_info', array( __CLASS__, 'product_taxonomy_info' ) );
	}
	...
	public static function product_taxonomy_info( $atts ) {
		$atts = shortcode_atts( array(
			...
			'classes'      			=> '',
		), $atts, 'dimax_' . __FUNCTION__ );

		$css_class = array(
			'dimax-product-taxonomy-infor',
			$atts['classes'],
		);
		...
		if ( $atts['show_thumbnail'] != 'no' ) {
			...
			if ( ! empty( $image ) ) {
				$image = '<div class="dimax-product-taxonomy-infor__image"><a href="'. esc_url( get_term_link( $term->term_id, $atts['taxonomy'] ) ) .'">'. $image .'</a></div>';
			}
		}

		if ( $atts['show_title'] != 'no' ) {
			$title = '<h4 class="dimax-product-taxonomy-infor__title"><a href="'. esc_url( get_term_link( $term->term_id, $atts['taxonomy'] ) ) .'">'. $term->name .'</a></h4>';
		}

		if ( $atts['show_description'] != 'no' ) {
			$description = '<div class="dimax-product-taxonomy-infor__description">'. $term->description .'</div>';
		}

		return sprintf(
			'<div class="%s">
			%s
			<div class="dimax-product-taxonomy-infor__content">%s%s</div>
			</div>',
