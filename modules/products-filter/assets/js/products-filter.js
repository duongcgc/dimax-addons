(function ($, _) {
    var RazziProductsFilterWidget = function () {
        ...
        $(document.body)
            ...
            .on('dimax_products_filter_change', {widget: self}, self.instantSearch)
            .on('dimax_products_filter_request_success', self.updateURL)
            .on('dimax_products_filter_request_success', {widget: self}, self.updateForm)
            .on('dimax_products_filter_widget_updated', {widget: self}, self.initDropdowns)
    ...
    RazziProductsFilterWidget.prototype.sendAjaxRequest = function (form) {
        ...
        $container.addClass('loading').append('<li class="dimax-posts__loading"><span class="dimax-loading"></span> </li>');

        $(document.body).trigger('dimax_products_filter_before_send_request', $container);
        ...
        self.ajax = $.get(url, function (response) {
            ...
            if (!$products.length) {
                ...
                $container.replaceWith($products);
                $products.find('li.product').addClass('animated dimaxFadeInUp');

                $(document.body).trigger('dimax_products_loaded', [$products.children(), false]); // appended = false
            }

            $form.removeClass('filtering');
            $(document.body).trigger('dimax_products_filter_request_success', [response, url, form]);
        });
    }
