...
class Widget extends \WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $instance['filter'] ) ) {
			...
			echo '<button type="submit" value="' . esc_attr__( 'Filter', 'dimax' ) . '" class="button filter-button">' . esc_html__( 'Filter', 'dimax' ) . '</button>';
			echo '<button type="reset" value="' . esc_attr__( 'Reset Filter', 'dimax' ) . '" class="button alt reset-button button-lg">' . esc_html__( 'Reset Filter', 'dimax' ) . '</button>';
			echo '</div>';
		...
		foreach ( $active_filters as $filter ) {
			...
			foreach ( $terms as $term ) {
				switch ( $filter['source'] ) {
					...
					case 'rating':
						$text = _n( 'Rated %d star', 'Rated %d stars', $term, 'dimax' );
	...
	public function display_filter( $filter ) {
		...
		if ( 'attribute' == $filter['source'] ) {
			...
			$args['all']        = sprintf( esc_html__( 'Any %s', 'dimax' ), wc_attribute_label( $attr->attribute_label ) );
			...
			$args['all'] = sprintf( esc_html__( 'Select a %s', 'dimax' ), $taxonomy->labels->singular_name );
		} else {
			$args['all'] = esc_html__( 'All Products', 'dimax' );
		}
        ...
        <div class="<?php echo esc_attr( join( ' ', $classes ) ) ?>">
            ...
            <div class="products-filter__filter-control filter-control">
                ...
                <div class="products-filter__control-buttons">
                    <button type="button" value="<?php esc_attr_e( 'Clear', 'dimax' ); ?>"
                            class="button alt clear-button"><?php esc_html_e( 'Clear', 'dimax' ); ?></button>
                    <button type="submit" value="<?php esc_attr_e( 'Show Results', 'dimax' ) ?>"
                            class="button filter-button button-lg"><?php esc_html_e( 'Show Results', 'dimax' ); ?></button>
                </div>
            </div>
        </div>
	...
	protected function get_filter_options( $filter ) {
		...
		switch ( $filter['source'] ) {
			...
			case 'products_group':
				$filter_groups = apply_filters( 'dimax_products_filter_groups', array(
				...
				if ( 'dropdown' != $filter['display'] ) {
					$options[''] = array(
						'name'  => esc_html__( 'All Products', 'dimax' ),
						'count' => 0,
						'id'    => 0,
						'level' => 0,
					);
			...
			case 'orderby':
				$orderby = apply_filters( 'dimax_products_filter_orderby', array(
					'menu_order' => esc_html__( 'Default sorting', 'dimax' ),
					'popularity' => esc_html__( 'Sort by popularity', 'dimax' ),
					'rating'     => esc_html__( 'Sort by average rating', 'dimax' ),
					'date'       => esc_html__( 'Sort by latest', 'dimax' ),
					'price'      => esc_html__( 'Sort by price: low to high', 'dimax' ),
					'price-desc' => esc_html__( 'Sort by price: high to low', 'dimax' ),
				) );
			...
			case 'product_status':
				...
				if(! isset($filter['disable_in_stock'])) {
					$options[ 'in_stock' ] = array(
						'name'  => esc_html__( 'In Stock', 'dimax' ),
						'count' => 0,
						'id'    => 'in_stock',
						'level' => 0,
					);
	...
	protected function filter_search_box( $filter ) {
		if ( 'attribute' == $filter['source'] ) {
			...
			$placeholder = __( 'Search', 'dimax' ) . ' ' . strtolower( $attributes[ $filter['attribute'] ] );
		} else {
			$sources     = $this->get_filter_source_options();
			$placeholder = __( 'Search', 'dimax' ) . ' ' . strtolower( $sources[ $filter['source'] ] );
		}
	...
	public function form( $instance ) {
        ...
        <div class="dimax-products-filter-form__sub-nav">
            <button type="button" data-section="filters"
                    class="button-link active"><?php esc_html_e( 'Filters', 'dimax' ); ?></button>
            |
            <button type="button" data-section="options"
                    class="button-link"><?php esc_html_e( 'Options', 'dimax' ); ?></button>
        </div>

        <p>
        <hr/></p>

        <div class="dimax-products-filter-form__section active" data-section="filters">
            <p class="dimax-products-filter-form__message <?php echo ! empty( $instance['filter'] ) ? 'hidden' : '' ?>"><?php esc_html_e( 'There is no filter yet.', 'dimax' ) ?></p>

            <div class="dimax-products-filter-form__filter-fields">
				<?php $this->filter_setting_fields( $instance['filter'] ); ?>
            </div>

            <p class="dimax-products-filter-form__section-actions">
                <button type="button" class="dimax-products-filter-form__add-new button-link"
                        data-name="<?php echo esc_attr( $this->get_field_name( 'filter' ) ); ?>"
                        data-count="<?php echo count( $instance['filter'] ) ?>">
                    + <?php esc_html_e( 'Add a new filter', 'dimax' ) ?></button>
            </p>
        </div>

        <div class="dimax-products-filter-form__section" data-section="design">
        </div>

        <div class="dimax-products-filter-form__section" data-section="options">
			<?php
			$this->setting_field( array(
				'type'  => 'checkbox',
				'name'  => 'ajax',
				'label' => esc_html__( 'Use ajax for filtering', 'dimax' ),
				'value' => $instance['ajax'],
			) );
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'instant',
				'label'     => esc_html__( 'Filtering products instantly (no buttons required)', 'dimax' ),
			...
			$this->setting_field( array(
				'type'      => 'select',
				'name'      => 'filter_buttons',
				'label'     => esc_html__( 'Filter buttons position', 'dimax' ),
				'value'     => $instance['filter_buttons'],
				'options'   => array(
					'form'   => __( 'Bottom of form', 'dimax' ),
					'fitems' => __( 'Bottom of filter items', 'dimax' ),
				),
				'condition' => array(
					'instant' => false,
				),
			...
			$this->setting_field( array(
				'type'      => 'checkbox',
				'name'      => 'change_url',
				'label'     => esc_html__( 'Update URL', 'dimax' ),
	...
	protected function filter_setting_fields( $fields = array(), $context = 'display' ) {
		...
		foreach ( $filter_fields as $index => $field ) :
			...
			?>
            <div class="dimax-products-filter-form__filter">
                <div class="dimax-products-filter-form__filter-top">
                    <div class="dimax-products-filter-form__filter-actions">
                        <button type="button"
                                class="dimax-products-filter-form__remove-filter button-link button-link-delete">
                            <span class="screen-reader-text"><?php esc_html_e( 'Remove filter', 'dimax' ) ?></span>
                            <span class="dashicons dashicons-no-alt"></span>
                        </button>
                    </div>

                    <button type="button" class="dimax-products-filter-form__filter-toggle">
                        <span class="dimax-products-filter-form__filter-toggle-indicator" aria-hidden="true"></span>
                    </button>

                    <div class="dimax-products-filter-form__filter-title"><?php echo $title; ?></div>
                </div>
                <div class="dimax-products-filter-form__filter-options">
					<?php
					foreach ( $filter_settings as $name => $options ) {
						...
						$options['class']      = 'dimax-products-filter-form__filter-option';
	...
	protected function get_filter_source_options() {
		$sources = array(
			'orderby'        => esc_html__( 'Order By', 'dimax' ),
			'products_group' => esc_html__( 'Group', 'dimax' ),
			'price'          => esc_html__( 'Price', 'dimax' ),
			'attribute'      => esc_html__( 'Attributes', 'dimax' ),
			'rating'         => esc_html__( 'Rating', 'dimax' ),
			'product_status' => esc_html__( 'Product Status', 'dimax' ),
		);
	...
	protected function get_filter_display_options( $source = 'product_cat' ) {
		$options = array(
			'price'     => array(
				...
				'ranges' => esc_html__( 'Ranges', 'dimax' ),
			),
			'attribute' => array(
				'auto'       => esc_html__( 'Auto', 'dimax' ),
				'dropdown'   => esc_html__( 'Dropdown', 'dimax' ),
				'list'       => esc_html__( 'Vertical List', 'dimax' ),
				'h-list'     => esc_html__( 'Horizontal List', 'dimax' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'dimax' ),
			),
			'rating'    => array(
				'dropdown'   => esc_html__( 'Dropdown', 'dimax' ),
				'checkboxes' => esc_html__( 'Checkbox List', 'dimax' ),
	...
	protected function get_filter_fields_settings() {
		...
		$this->filter_settings = array(
			'name'        => array(
				'type'  => 'text',
				'label' => __( 'Filter Name', 'dimax' ),
			),
			'source'      => array(
				'type'    => 'select',
				'label'   => __( 'Filter By', 'dimax' ),
				'options' => $this->get_filter_source_options(),
			),
			'attribute'   => array(
				'type'      => 'select',
				'name'      => 'attribute',
				'label'     => __( 'Attribute', 'dimax' ),
			...
			'display'     => array(
				'type'    => 'select',
				'label'   => __( 'Display Type', 'dimax' ),
				'options' => $this->get_filter_display_options(),
			),
			'ranges'      => array(
				'type'      => 'textarea',
				'label'     => __( 'Ranges', 'dimax' ),
				'desc'      => __( 'Each range on a line, separate by the <code>-</code> symbol. Do not include the currency symbol.', 'dimax' ),
				'condition' => array(
					'display' => 'ranges',
					'source'  => 'price',
				),
			...
			'multiple'    => array(
				'type'      => 'select',
				'label'     => __( 'Selection Type', 'dimax' ),
				'options'   => array(
					0 => __( 'Single select', 'dimax' ),
					1 => __( 'Multiple select', 'dimax' ),
				),
				'condition' => array(
			...
			'show_counts' => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show product counts', 'dimax' ),
				'condition' => array(
			...
			'searchable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show the search box', 'dimax' ),
				'condition' => array(
			...
			'scrollable'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Limit the height of items list (scrollable)', 'dimax' ),
				'condition' => array(
					'source!'  => array( 'product_status' ),
			...
			'show_empty_cats'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Show empty categories', 'dimax' ),
				'condition' => array(
					'source'  => array( 'product_cat' ),
				),
			),
			'disable_onsale'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable on sale', 'dimax' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
			'disable_in_stock'  => array(
				'type'      => 'checkbox',
				'label'     => __( 'Disable in stock', 'dimax' ),
				'condition' => array(
					'source'  => array( 'product_status' ),
				),
			),
	...
	public function filter_setting_fields_template() {
        ...
        <script type="text/template" id="tmpl-dimax-products-filter">
