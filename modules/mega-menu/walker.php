...
class Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_is_mega         = apply_filters( 'dimax_menu_item_mega', get_post_meta( $item->ID, 'tamm_menu_item_mega', true ), $item->ID );
		...
		if ( $item_icon_type === 'svg' ) {
			...
			$item_icon_html  = ! empty( $item_icon_svg ) ? '<span ' . $item_icon_color . ' class="dimax-svg-icon">' . \Dimax\Icon::sanitize_svg( $item_icon_svg ) . '</span> ' : '';
		...
		if ( $item_badges_text ) {
			...
			$item_badges_html = '<span ' . $item_badges_style . ' class="dimax-menu-badges">' . $item_badges_text . '</span>';
		}
