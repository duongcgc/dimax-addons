...
class Frontend {
	...
	public function hide_price() {
		...
		if ( get_option( 'dimax_product_loop_hide_price' ) == 'yes' ) {
			remove_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_price', 10 );
			add_filter( 'dimax_product_loop_show_price', '__return_false' );
		}

		// Hide price in the product page && quick view
		if ( get_option( 'dimax_product_hide_price' ) == 'yes' ) {
			remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_price', 10 );
			remove_action( 'dimax_woocommerce_single_product_summary', 'woocommerce_template_single_price', 10 );
			add_filter( 'dimax_product_show_price', '__return_false' );
		}

		// Hide price in the wishlist page
		if ( get_option( 'dimax_wishlist_hide_price' ) ) {
			add_filter( 'yith_wcwl_wishlist_params', array( $this, 'dimax_wishlist_hice_price' ) );
		}
	}
	 ...
	 * @return array
	 */
	public function dimax_wishlist_hice_price( $params ) {
	...
	public function hide_add_to_cart() {
		...
		if ( get_option( 'dimax_product_loop_hide_atc' ) == 'yes' ) {
			add_filter( 'dimax_get_product_loop_featured_icons', array( $this, 'product_loop_featured_icons' ) );
			add_filter( 'dimax_product_loop_show_atc', '__return_false' );
		}

		// Hide Add to Cart in the product page & quick view
		if ( get_option( 'dimax_product_hide_atc' ) == 'yes' ) {
			...
			add_action( 'dimax_woocommerce_product_quickview_summary', array( $this, 'add_to_wishlist_button' ), 80 );

		}

		// Hide add to cart in the wishlist page
		if ( get_option( 'dimax_wishlist_hide_atc' ) ) {
	...
	public function add_to_wishlist_button() {
		...
		echo '<div class="rz-wishlist-button dimax-button button-outline show-wishlist-title">';
	...
	public function check_page_redirect() {
		...
		if ( ( $cart && get_option( 'dimax_hide_cart_page' ) == 'yes' ) || ( $checkout && get_option( 'dimax_hide_checkout_page' ) == 'yes' ) ) {
	...
	public function remove_woo_page_links( $pages ) {
		$cart     = get_option( 'dimax_hide_cart_page' ) == 'yes' ? wc_get_page_id( 'cart' ) : '';
		$checkout = get_option( 'dimax_hide_checkout_page' ) == 'yes' ? wc_get_page_id( 'checkout' ) : '';
	...
	public function remove_woo_action_buttons() {
		...
		if ( get_option( 'dimax_hide_cart_page' ) == 'yes' ) {
			add_filter( 'dimax_mini_cart_button_view_cart', '__return_false' );
		}

		if ( get_option( 'dimax_hide_checkout_page' ) == 'yes' ) {
			add_filter( 'dimax_mini_cart_button_proceed_to_checkout', '__return_false' );
		}
	}
