...
class Product_Authors {
	...
	public function __construct() {
		...
		$this->active_author = apply_filters( 'dimax_register_author', true );
	...
	public function register_author() {
		...
		$labels = array(
			'name'                       => __( 'Product Authors', 'dimax' ),
			'singular_name'              => __( 'Author', 'dimax' ),
			'menu_name'                  => __( 'Authors', 'dimax' ),
			'all_items'                  => __( 'All Authors', 'dimax' ),
			'edit_item'                  => __( 'Edit Author', 'dimax' ),
			'view_item'                  => __( 'View Author', 'dimax' ),
			'update_item'                => __( 'Update Author', 'dimax' ),
			'add_new_item'               => __( 'Add New Author', 'dimax' ),
			'new_item_name'              => __( 'New Author Name', 'dimax' ),
			'parent_item'                => __( 'Parent Author', 'dimax' ),
			'parent_item_colon'          => __( 'Parent Author:', 'dimax' ),
			'search_items'               => __( 'Search Authors', 'dimax' ),
			'popular_items'              => __( 'Popular Authors', 'dimax' ),
			'separate_items_with_commas' => __( 'Separate Authors with commas', 'dimax' ),
			'add_or_remove_items'        => __( 'Add or remove Authors', 'dimax' ),
			'choose_from_most_used'      => __( 'Choose from the most used Authors', 'dimax' ),
			'not_found'                  => __( 'No Authors found', 'dimax' )
		);

		$permalinks         = get_option( 'product_author_permalinks' );
		$product_author_base = empty( $permalinks['product_author_base'] ) ? _x( 'product-author', 'slug', 'dimax' ) : $permalinks['product_author_base'];
	...
	public function settings_api_init() {
		add_settings_section(
			'dimax_author_section',
			'<span id="author-options">' . esc_html__( 'Product author', 'dimax' ) . '</span>',
		...
		add_settings_field(
			$this->option,
			'<span class="author-options">' . esc_html__( 'Product author', 'dimax' ) . '</span>',
		...
		add_settings_field(
			'product_author_slug',
			'<label for="product_author_slug">' . esc_html__( 'Product author base', 'dimax' ) . '</label>',
	...
	public function product_author_slug_input() {
        ...
        <input name="product_author_slug" type="text" class="regular-text code"
               ...
               placeholder="<?php echo esc_attr_x( 'product-author', 'slug', 'dimax' ) ?>"/>
		<?php
	}
	...
	public function product_author_column( $columns, $column, $id ) {
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'dimax' ) . '" class="wp-post-image" height="48" width="48" />';
	...
	public function disable_field_html() {
        ...
        <label for="<?php echo esc_attr( $this->option ); ?>">
			...
			<?php esc_html_e( 'Enable Author for this site.', 'dimax' ); ?>
        </label>
