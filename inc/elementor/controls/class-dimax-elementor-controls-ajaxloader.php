	...
	public function ra_autocomplete_taxonomy_callback( $taxonomy = 'category' ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = esc_html__( 'Id', 'dimax' ) . ': ' . $value['id'] . ' - ' . esc_html__( 'Name', 'dimax' ) . ': ' . $value['name'];
				$result[]      = $data;
			}
		} else {
			$result[] = array(
				'value' => 'nothing-found',
				'label' => esc_html__( 'Nothing Found', 'dimax' )
			);
		}
	...
	public function ra_autocomplete_post_type_callback( $post_type = 'product' ) {
		...
		if ( is_array( $posts ) && ! empty( $posts ) ) {
			foreach ( $posts as $post ) {
				$data          = array();
				$data['value'] = $post->ID;
				$data['label'] = esc_html__( 'Id', 'dimax' ) . ': ' . $post->ID . ' - ' . esc_html__( 'Title', 'dimax' ) . ': ' . $post->post_title;
				$result[]      = $data;
			}
		} else {
			$result[] = array(
