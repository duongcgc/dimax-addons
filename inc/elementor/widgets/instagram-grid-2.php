...
class Instagram_Grid_2 extends Widget_Base {
	...
	protected function section_content() {
		$this->start_controls_section(
			'section_content',
			[ 'label' => esc_html__( 'Instagram', 'dimax' ) ]
		);

		$this->add_control(
			'instagram_type',
			[
				'label' => esc_html__( 'Instagram type', 'dimax' ),
		...
		$repeater->add_control(
			'caption',
			[
				'label' => esc_html__( 'Caption', 'dimax' ),
				'type' => Controls_Manager::TEXTAREA,
				'dynamic' => [
					'active' => true,
				],
				'default' => '',
				'placeholder' => esc_html__( 'Enter your caption', 'dimax' ),
				'rows' => 4,
			]
		);
		...
		$this->add_control(
			'image_list',
			[
				'label'         => esc_html__( 'Image List', 'dimax' ),
		...
		$this->add_control(
			'link_type',
			[
				'label'   => esc_html__( 'Button Link Type', 'dimax' ),
				'type'    => Controls_Manager::SELECT,
				'options' => [
					'default'   => esc_html__( 'Default user', 'dimax' ),
					'custom' 	 => esc_html__( 'Custom', 'dimax' ),
				],
				'default' => 'default',
				'toggle'  => false,
	...
	protected function section_style() {
		$this->start_controls_section(
			'style_general',
			[
				'label' => __( 'Instagram Content Box', 'dimax' ),
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-text-wrapper' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-text-wrapper' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .dimax-icon' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .dimax-icon' => 'font-size: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .dimax-icon' => 'color: {{VALUE}};',
				],
				'condition' => [
					'instagram_icon_type' => ['custom'],
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-title' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .dimax-instagram-grid-2 .instagram-title',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-title' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-desc' => 'margin-bottom: {{SIZE}}{{UNIT}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .dimax-instagram-grid-2 .instagram-desc',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .instagram-desc' => 'color: {{VALUE}};',
				],
			]
		);
		$this->add_control(
		...
		$this->add_group_control(
			...
			[
				'name'     => 'btn_typography',
				'selector' => '{{WRAPPER}} .dimax-instagram-grid-2 .button-text',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid-2 .button-text' => 'color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute(
			'wrapper', 'class', [
				'dimax-instagram-grid-2',
				'dimax-element-columns',
			]
		);
		...
		if ( $settings['instagram_icon_type'] == 'custom' && $settings['icon'] && ! empty( $settings['icon']['value'] ) && \Elementor\Icons_Manager::is_migration_allowed() ) {
			...
			$instagram_icon = '<span class="dimax-icon '.$add_class_icon.'">' . ob_get_clean() . '</span>';
		}

		$button_text = $settings['button_text'] ? sprintf('<span class="button-text dimax-button--underlined">%s</span>',$settings['button_text']) : '';
