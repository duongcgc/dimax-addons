...
class Dimax_Product extends Widget_Base {
	...
	public function get_name() {
		return 'dimax-product-shortcode';
	}
	...
	protected function section_products_settings_controls() {
		...
		$this->add_control(
			'show_lightbox',
			[
				'label'     => esc_html__( 'Lightbox', 'dimax' ),
				'type'      => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'dimax' ),
				'label_on'  => __( 'Show', 'dimax' ),
				'return_value' => 'show',
		...
		$this->add_control(
			'show_size_chart',
			[
				'label'     => esc_html__( 'Size Chart', 'dimax' ),
				'type'      => Controls_Manager::SWITCHER,
				'label_off' => __( 'Hide', 'dimax' ),
				'label_on'  => __( 'Show', 'dimax' ),
				'return_value' => 'show',
	...
	protected function section_content_style_controls() {
		...
		$this->add_control(
			'content_divider',
			[
				'label' => esc_html__( 'Content Wrapper', 'dimax' ),
				'type'  => Controls_Manager::HEADING,
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .dimax-product-shortcode' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-product-shortcode' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			'content_divider_2',
			[
				'label' => esc_html__( 'Content Inner', 'dimax' ),
				'type'  => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', 'em', '%' ],
				'selectors'  => [
					'{{WRAPPER}} .dimax-product-shortcode .rz-product-wrapper' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-product-shortcode .rz-product-wrapper' => 'background-color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		$classes = [
			'dimax-product-shortcode single-product woocommerce dimax-swiper-carousel-elementor'
		];
		...
		if ( empty( $product ) ) {
			echo esc_html__( 'No products were found matching your selection.', 'dimax' );
		} else{
			...
			if ( $settings['show_description'] != '' ) {
				add_action( 'dimax_woocommerce_single_product_summary', 'woocommerce_template_single_excerpt', 20 );
			}
