...
class Instagram_Grid extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Dimax - Instagram Grid', 'dimax' );
	}
	...
	protected function section_content() {
		...
		$this->add_control(
			'text',
			[
				'label'       => esc_html__( 'Profile Text', 'dimax' ),
				'type'        => Controls_Manager::TEXT,
				'default'     => esc_html__( 'This is text', 'dimax' ),
			]
		);
		...
		$this->add_control(
			'username',
			[
				'label'       => esc_html__( 'Username', 'dimax' ),
				'type'        => Controls_Manager::TEXT,
				'placeholder' => __( 'username', 'dimax' ),
				'conditions' => [
					'terms' => [
						[
							'name' => 'instagram_type',
	...
	protected function section_style() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors'  => [
					'{{WRAPPER}} .dimax-instagram-grid .instagram-wrapper' => 'margin: calc(-{{SIZE}}{{UNIT}}/2);',
					'{{WRAPPER}} .dimax-instagram-grid .instagram-item' => 'padding:calc({{SIZE}}{{UNIT}}/2);',
				],
			]
		);
		...
		$this->add_control(
			'text_box_style',
			[
				'label' => esc_html__( 'Text Box', 'dimax' ),
				'type'  => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name'     => 'box_typography',
				'selector' => '{{WRAPPER}} .dimax-instagram-grid .instagram-text-box',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid .instagram-text-box' => 'background-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'{{WRAPPER}} .dimax-instagram-grid .instagram-text-box' => 'color: {{VALUE}};',
				],
			]
		);
	...
	protected function render() {
		...
		if ( $settings['instagram_type'] === 'token' ) {
			...
			$user = apply_filters( 'dimax_get_instagram_user', array() );
			if ( empty( $user ) ) {
			...
			$output[] = sprintf('</ul>');
			$output[] = $settings["text"] ? '<a href="https://www.instagram.com/'.$user['username'].'" class="instagram-text-box dimax-button button-larger" target="_blank">' . $settings["text"] .'</a>' : '';
		} else {
			...
			$output[] = sprintf('</ul>');
			$output[] = $settings["text"] ? '<a href="https://www.instagram.com/'. strtolower($settings['username']) .'" class="instagram-text-box dimax-button button-larger" target="_blank">' . $settings["text"] .'</a>' : '';
		}
