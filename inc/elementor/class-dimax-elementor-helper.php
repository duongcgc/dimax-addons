...
class Helper {
	...
	public static function products_shortcode( $atts ) {
		...
		if ( in_array( $type, array( 'day', 'week', 'month' ) ) ) {
			...
			$query_args['meta_query'] = apply_filters(
				'dimax_product_deals_meta_query', array_merge(
	...
	public static function get_transient_name( $query_args, $type ) {
		$transient_name = 'dimax_product_loop_' . md5( wp_json_encode( $query_args ) . $type );
	...
	public static function get_recently_viewed_products( $settings ) {
		...
		if ( empty( $product_ids ) ) {
			...
			printf(
				...
				self::control_url( 'empty_button', $settings['button_link'], $settings['button_text'], [ 'class' => 'dimax-button' ] )
			);
	...
	public static function get_instagram_get_photos_by_token( $limit, $access_token ) {
		if ( empty( $access_token ) ) {
			return new \WP_Error( 'instagram_no_access_token', esc_html__( 'No access token', 'dimax' ) );
		}
		...
		if ( isset( $user['error'] ) ) {
			return new \WP_Error( 'instagram_no_images', esc_html__( 'Instagram did not return any images. Please check your access token', 'dimax' ) );

		} else {
			$transient_key = 'dimax_instagram_photos_' . sanitize_title_with_dashes( $user['username'] . '__' . $limit );
			$images        = get_transient( $transient_key );
			$images = apply_filters( 'dimax_get_instagram_photos', $images, $access_token );
			...
			if ( ! empty( $images ) ) {
				...
				return new \WP_Error( 'instagram_no_images', esc_html__( 'Instagram did not return any images.', 'dimax' ) );
			}
		}
	}
	...
	public static function get_instagram_user( $access_token ) {
		if ( empty( $access_token ) ) {
			return new \WP_Error( 'no_access_token', esc_html__( 'No access token', 'dimax' ) );
		}
		$transient_key = 'dimax_instagram_user_' . $access_token;
		$user = get_transient( $transient_key);
		$user = apply_filters( 'dimax_get_instagram_user', $user );
		if ( false === $user ) {
			$url  = add_query_arg( array(
				'fields'       => 'id,username',
				'access_token' => $access_token
			...
			if ( ! $data ) {
				return new \WP_Error( 'no_user_data', esc_html__( 'No user data received', 'dimax' ) );
			}
